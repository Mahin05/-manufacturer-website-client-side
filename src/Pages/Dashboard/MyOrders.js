...
const MyOrders = () => {
    ...
    useEffect(() => {
        if (user) {
            fetch(`https://manufacturer-website-server-side-zd8v.onrender.com/booking?email=${user.email}`, {
                method: 'GET',
                headers: {
    ...
    const handleDelete = id => {
        ...
        if (proceed) {
            const url = `https://manufacturer-website-server-side-zd8v.onrender.com/inventory/${id}`
            fetch(url, {
                method: 'DELETE'
            })
                .then(res => res.json())
